...
namespace Infrastructure.Services
{
    public class ExamService : IExamService
    {
        ...
        public ExamService(IUnitOfWork unitOfWork
            //, IMapper mapper
            , IHttpContextAccessor httpContextAccessor, ICourseService courseService)
        {
        ...
        public async Task<ResultDTO> AddExam(AddExamDTO examDTO)
        {
            if (examDTO != null)
            {
                ...
                Instructor member = _unitOfWork.InstructorRepo.Get(x => x.UserId == int.Parse(userName));
                exam = examDTO.ExamDTO.MapOne<Exam>();
                ...
                List<Choice> choices = new List<Choice>();
                    ...
                    question = _question.MapOne<Question>();
        ...
        public async Task<ResultDTO> AddExamBySystem()
        {
            ...
            int randomLevel = random.Next(0,2);
            List<Question> questionList = new List<Question>();
                List<ExamQuestion> examQuestions = new List<ExamQuestion>();
                ...
                List<Choice> choices = new List<Choice>();
                    ...
                    question.grade = 10;
                    question.ExamId = exam.id;
                    question.Level = levels[randomLevel];
        ...
        public async Task<ResultDTO> GetExams()
        {
            ...
            List<Exam> exams =  _unitOfWork.ExamRepo.GetAll(e=>e.InstructorId == member.id).ToList();
        ...
        public async Task<ResultDTO> TakeExam(TakeExamDTO takeExamDTO)
        {
            if(takeExamDTO != null)
            {
                ...
                string UserName = _httpContextAccessor.HttpContext.Session.GetString("Username");
                Student student = _unitOfWork.StudentRepo.Get(s=>s.UserId == int.Parse(UserName));
        ...
        public async Task<ResultDTO> ViewResult(ExamResultDTO examResultDTO)
        {
            if(examResultDTO != null)
            {
                ...
                Instructor instructor = _unitOfWork.InstructorRepo.Get(s => s.UserId == int.Parse(UserNameInstructor));
