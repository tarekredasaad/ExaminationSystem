...
namespace Infrastructure.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<ResultDTO> RegisterAsyncUser(UserDTO dto)
        {
        ...
        public async Task<ResultDTO> LoginAsyncUser(UserLoginDTO dto)
        {
            User user = _unitOfWork.UserRepository.Get(i => i.username == dto.username);
                var password = HashPassword.Decrypt( user.password);
            if (user == null || password != dto.password)
            {
            ...
            _httpContextAccessor.HttpContext.Session.SetString("Username", user.id.ToString());
            return (new ResultDTO() { StatusCode = 200, Data = user, Message = "you login successfully" });
        }
